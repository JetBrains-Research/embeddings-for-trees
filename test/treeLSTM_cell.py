...
def _gen_node_with_children(number_of_children: int, edges_to_root: bool = False) -> dgl.DGLGraph:
...
class TreeLSTMCellTest(unittest.TestCase):
    ...
    def test_childsum_tree_lstm_messages(self):
        device = get_device()
        fix_seed()
        ...
        for x_size, h_size, number_of_children in tqdm(zip(x_sizes, h_sizes, numbers_of_children), total=len(x_sizes)):
            ...
            g = _gen_node_with_children(number_of_children, True)
